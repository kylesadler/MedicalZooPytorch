...
def datestr():
    ...
    return "{:02}-{:02}_{:02}-{:02}".format(
        now.tm_mday, now.tm_mon, now.tm_hour, now.tm_min
    )
...
def get_parser(**kwargs):
    ...
    parser.add_argument(
        "--lr", default=kwargs["lr"], type=float, help="learning rate (default: 1e-3)"
    )
...
def to_args(kwargs):
    ...
    args.save = f"/home/kyle/MedicalZooPytorch/results/{args.model}/{args.dataset_name}_{args.fold_id}_{datestr()}"
    return args
