...
from medzoo.lib.medloaders.medical_loader_utils import (
    create_sub_volumes,
    create_non_overlapping_sub_volumes,
)
...
class MICCAIBraTS2019(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_seg = (
            np.equal(img_seg, 4).astype(int) * 3
            + np.equal(img_seg, 1).astype(int) * 2
            + np.equal(img_seg, 2).astype(int)
        )

        if self.mode == "train" and self.augmentation:
            [img_t1, img_t1ce, img_t2, img_flair], img_seg = self.transform(
                [img_t1, img_t1ce, img_t2, img_flair], img_seg
            )

            return (
                torch.FloatTensor(img_t1.copy()).unsqueeze(0),
                torch.FloatTensor(img_t1ce.copy()).unsqueeze(0),
                torch.FloatTensor(img_t2.copy()).unsqueeze(0),
                torch.FloatTensor(img_flair.copy()).unsqueeze(0),
                torch.FloatTensor(img_seg.copy()),
            )
